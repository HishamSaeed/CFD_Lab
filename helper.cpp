...
#define READ_ERROR(szMessage, szVarName, szFileName, nLine)                    \
  {                                                                            \
    char szTmp[80];                                                            \
    if (nLine)                                                                 \
      sprintf(szTmp, " %s  File: %s   Variable: %s  Line: %d", szMessage,      \
              szFileName, szVarName, nLine);                                   \
    else                                                                       \
      sprintf(szTmp, " %s  File: %s   Variable: %s ", szMessage, szFileName,   \
              szVarName);                                                      \
    ERROR(szTmp);                                                              \
  }
...
char *find_string(const char *szFileName, const char *szVarName) {
  ...
  while (!feof(fh)) {
    ...
    /* remove empty lines */
    while (isspace((int)*szLine) && *szLine)
      ++szLine;
    if (strlen(szLine) == 0)
      continue;
    ...
    szName = szLine;
    szValue = szLine;
    while ((isalnum((int)*szValue) || *szValue == '_') && *szValue)
      ++szValue;
...
void read_string(const char *szFileName, const char *szVarName,
                 char *pVariable) {
  char *szValue = NULL; /* string containg the read variable value */

  if (szVarName == 0)
    ERROR("null pointer given as variable name");
  if (szFileName == 0)
    ERROR("null pointer given as filename");
  if (pVariable == 0)
    ERROR("null pointer given as variable");
  ...
  printf("File: %s\t\t%s%s= %s\n", szFileName, szVarName,
         &("               "[min_int(strlen(szVarName), 15)]), pVariable);
}
...
void read_double(const char *szFileName, const char *szVarName,
                 double *pVariable) {
...
/* --------------------------------------------------------------------------*/
std::pair<double, int> read_file_regex(std::string filename, std::string sZName,
                                       read_type type) {
  ...
  std::regex number_float("[+-]?\\d+\\.\\d{1,6}");
...
void get_file_double(std::string filename, std::string sZName, double &value) {
  std::pair<double, int> res =
      read_file_regex(filename, sZName, read_type::DOUBLE);
  value = std::get<double>(res);
}

void get_file_int(std::string filename, std::string sZName, int &value) {
  std::pair<double, int> res =
      read_file_regex(filename, sZName, read_type::INT);
...
int **read_pgm(const char *filename) {
  ...
  if (strncmp("P2", line, 2) != 0) {
    ERROR("file is not a valid PGM file. If file is a valid PGM file, first "
          "two characters of the first line must read P2");
  }
...
void check_dir_exists(std::string outputfolder) {
  ...
  // if the directory is provided like "results", the top level directory
  // doesn't need to be used
  else {
    std::cout << "Folder: " << folder << " doesn't exists." << std::endl;
    if (filesystem::create_directory(folder))
      std::cout << "Folder: " << folder << " created successfully."
                << std::endl;
  }
}
